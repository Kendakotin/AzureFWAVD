...
function render(list){
  ...
  root.innerHTML = list.map(p => `
    <article class="project" id="${p.id}">
      ...
      <div class="section-title">Low-level architecture diagram</div>
      <img class="arch" src="${(p.architectureImage||'').replace('assets/diagrams/','assets/lowlevel/').replace('.svg','_lowlevel.svg')}" alt="${p.title} low-level architecture diagram"/>
